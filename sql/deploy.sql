...
-- 1. Template table
-- ============================================================
CREATE TABLE IF NOT EXISTS SisulaTemplates (
...
-- ============================================================
-- 2. Template CRUD stored procedure
-- ============================================================
...
AS $$
    ...
    if (op === 'UPSERT') {
        var mergeSql = `
            MERGE INTO SisulaTemplates AS t
            USING (SELECT ? AS n, ? AS c) AS s
            ON t.name = s.n
            WHEN MATCHED THEN UPDATE SET content = s.c, modified_at = CURRENT_TIMESTAMP()
            WHEN NOT MATCHED THEN INSERT (name, content, modified_at) VALUES (s.n, s.c, CURRENT_TIMESTAMP())
        `;
        snowflake.createStatement({sqlText: mergeSql, binds: [name, content]}).execute();
        return 'OK';
    } else if (op === 'DELETE') {
        snowflake.createStatement({sqlText: 'DELETE FROM SisulaTemplates WHERE name = ?', binds: [name]}).execute();
        return 'OK';
    } else if (op === 'GET') {
        var rs = snowflake.createStatement({sqlText: 'SELECT content FROM SisulaTemplates WHERE name = ?', binds: [name]}).execute();
        if (rs.next()) {
        ...
        var sb = [];
        var rs = snowflake.createStatement({sqlText: 'SELECT name, modified_at FROM SisulaTemplates ORDER BY name'}).execute();
        while (rs.next()) {
...
-- ============================================================
-- 3. Helper: render a template by name from the templates table
-- ============================================================
...
AS $$
    var rs = snowflake.createStatement({
        sqlText: 'SELECT content FROM SisulaTemplates WHERE name = ?',
        binds: [TPL_NAME]
    }).execute();
    var template = null;
    if (rs.next()) {
...
return render(TEMPLATE, parseJSON(BINDINGS));
$$;
